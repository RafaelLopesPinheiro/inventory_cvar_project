#!/usr/bin/env python
"""
Rolling Window Experiment Runner for Inventory CVaR Optimization

This script runs experiments using rolling window cross-validation,
comparing all models (traditional + deep learning + TFT) across multiple
time windows.

Usage:
    python run_rolling_window_experiment.py [--epochs EPOCHS] [--output OUTPUT_DIR]
"""
...
def run_window_experiment(
    ...
    """
    Run experiment on a single rolling window.

    Parameters
    ----------
    window_split : RollingWindowSplit
        Rolling window split data.
    config : ExperimentConfig
...
def main(config: ExperimentConfig):
    """
    Main rolling window experiment runner.
    ...
    logger.info("=" * 70)
    logger.info("ROLLING WINDOW CVaR OPTIMIZATION EXPERIMENT")
    logger.info("=" * 70)
    ...
    logger.info(f"Created {len(rolling_splits)} rolling windows\n")

    # Run experiment on each window
    all_window_results = []

    for window_split in rolling_splits:
        summary_df = run_window_experiment(window_split, config)
    ...
    # Save results
    agg_path = os.path.join(config.results_dir, "rolling_window_aggregated.csv")
    aggregated.to_csv(agg_path)
    logger.info(f"\n[OK] Saved aggregated results: {agg_path}")

    all_path = os.path.join(config.results_dir, "rolling_window_all.csv")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Rolling Window CVaR Optimization Experiment")
    parser.add_argument("--output", type=str, default="results/rolling_window", help="Output directory")
